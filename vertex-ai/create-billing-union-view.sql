...
SELECT
  ...
  REGEXP_REPLACE(
    REGEXP_REPLACE(
      REGEXP_REPLACE(sku.description, r'(?i)^Cloud Vertex AI Model Garden Model as a Service ', ''),
      r' — .*', ''),
    r'(?i)\s+(Cached\s+)?(Thinking\s+)?(Text|Audio)?\s*(Input|Output)\s*(Token|Caching)?(\s.*)?$', ''
  ) AS sku_description,
